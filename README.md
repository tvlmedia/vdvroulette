...
Date Roulette is een mobile-first kaartspel voor twee spelers. Deze versie bevat de basisgame, LocalStorage-opslag, levelprogressie per speler, categorie-unlocks, timerkaarten, volledige Jacuzzi-modus, instellingen, statistieken, ontwikkelmodus en een herbruikbare engine voor speciale kaarten.
...
- `service-worker.js`: offline cache en updateafhandeling met cacheversie `date-roulette-v1`.
...
Na het eerste bezoek cachet `service-worker.js` de app-shell, kaartmodules, manifest en lokale iconen onder cacheversie `date-roulette-v1`. Navigatie gebruikt network-first, zodat online bezoeken verse HTML ophalen. Offline valt de app terug op de gecachte `index.html`; assets worden cache-first geladen en op de achtergrond bijgewerkt.
...
- `jacuzziAllowed`: `true` als een gewone kaart in Jacuzzi-modus mag verschijnen, `false` als dat onhandig is.
...
- `playerRestriction`: optioneel. Mogelijke waarden: `null`, `player_1`, `player_2`, `winnie`, `tijgertje`.
...
- `perfectRun`: vijf gewone kaarten achter elkaar, stop bij één skip.
...
De huidige state gebruikt `stateVersion: 3`. Oude opslag uit versie 1, 2 en 3 wordt bij openen automatisch gemigreerd.

Belangrijke velden:

- `players`: spelers met `id`, `name`, `completedCards` en `lipstickKisses`.
...
Voorbeeld: Level 3 is pas actief wanneer beide spelers minimaal 8 kaarten hebben afgerond. Als Winnie 12 kaarten heeft afgerond en Tijgertje 7, blijft het spel op Level 2.
...
- gewone kaarten mogen alleen verschijnen met `jacuzziAllowed: true`;
...
Special-kaarten gebruiken dezelfde eligibility-filters. Roulette, Flirty-keuze en Perfecte Run trekken dus geen kaarten die door level, Jacuzzi-modus of `playerRestriction` ongeschikt zijn.
...
`lighterText` is de daadwerkelijke opdracht voor `lighterVersion`. De oorspronkelijke skip blijft historisch bewaard; de lichtere versie wordt apart in `cardHistory` vastgelegd met variant `lighter`.

## Lippenstiftregels

Alle straffen lopen via `addLipstickKiss(playerIndex, reason)`. Daardoor krijgt één klik precies één kusje, ook binnen Roulette, Perfecte Run, Golden Card, Wild Card, upgrades en lichtere varianten.
...
- drie voorbeeld-skips toevoegen;
...
- Voeg voorbeeld-skips toe en test `Spelen met spanning` en `Lichtere versie`.
- Voeg voorbeeld-completions toe en test `Dubbel zo spannend`.
- Zet Jacuzzi-modus aan en controleer dat specials alleen geschikte gewone kaarten selecteren.
